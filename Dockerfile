...
RUN set -eux; \
    ...
    install -D -m0755 backends/zebra/target/release/zallet-zebra-state /out/zallet-zebra-state; \
...
COPY --from=builder /out/zallet-zebra-state /zallet-zebra-state
...
COPY --from=builder /out/zallet-zebra-state /usr/local/bin/zallet-zebra-state
COPY --from=builder /out/usr/local/share/zallet /usr/local/share/zallet
USER 1000:1000
